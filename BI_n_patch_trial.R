...
library(ggplot2)
library(MASHmacro)
source("Multipatch_data_transform.R")
set.seed(0)
...
########################################
source("Bioko_Island_Simulation_Setup.R")
...
directory = "BI_n_patch_trial_outputs/Single_trajectory"
...
# "BI_n_patch_trial_outputs/Single_trajectory_verify"

########################################
# Simulation Ensemble
########################################
# reset the tile, with a new directory
directory = "BI_n_patch_trial_outputs/Ensemble_trajectories"
tile = MacroTile$new(nPatch = n,AquaPar = aquaPar,PatchPar = patchPar,MosquitoPar = mosquitoPar,HumanPar = humanPar,directory = directory)
for (i in 1:10){
  set.seed(i + 3)
  tile$simMacro(tMax = 750, PfPAR = pfpr)
...
# Check your output against the output saved at:
# "BI_n_patch_trial_outputs/Ensemble_trajectories_verify"
